...
def test_model_installiation(model_config_fixture):
    ...
    try:
        ...
        assert isinstance(model, nn.Module), (
            "El modelo debe ser una instancia de torch.nn.Module"
        )
...
def test_model_architecture(model_config_fixture):
    ...
    assert len(net) == expected_layers_count, (
        f"Se esperaba {expected_layer_count}, habia {len(net)}."
    )
    # hidden layer 1
    assert (
        isinstance(net[0], nn.Linear)
        and net[0].in_features == 25
        and net[0].out_features == 128
    )
    ...
    # hidden layer 2
    assert (
        isinstance(net[4], nn.Linear)
        and net[4].in_features == 128
        and net[4].out_features == 64
    )
    ...
    # hidden layer 3
    assert (
        isinstance(net[8], nn.Linear)
        and net[8].in_features == 64
        and net[8].out_features == 1
    )
...
def test_forward_pass(model_config_fixture):
    ...
    assert output.shape == expected_shape, (
        f"la forma del tensor de salida es incorrecto, esperado: {expected_shape}. recivido: {output.shape}"
    )
